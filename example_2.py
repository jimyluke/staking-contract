...
#Simple example to show the pool creation, deposit and deletion because MAX_SECONDS_TO_CLAIM passed before the end.
#Then the depositor used withdraw
...
#To effectively run this example set in the deployed contract a small MAX_SECONDS_TO_CLAIM
MAX_SECONDS_TO_CLAIM = 10
POOL_DURATION = 60
...
def main():
    ...
    sleep(POOL_DURATION + MAX_SECONDS_TO_CLAIM)
    ...
    #Claim from the pool 
    client.submit_withdraw_from_pool(pool_id, depositor.pk, depositor.address)
    print("Withdrew from the pool {}".format(pool_id))
    print("Local State:")
    print(client.get_formatted_local_state(depositor.address), end="\n\n")
